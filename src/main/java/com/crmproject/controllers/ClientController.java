...
public class ClientController {
	...
	//getId
	@GetMapping("/{id}")
	public ResponseEntity<List<Client>> getByClientId(@PathVariable long id){
		 return ResponseEntity.ok(repository.findAllById(
				  id));
	}
	
	//getName
	@GetMapping("/{nameClient}")
	public ResponseEntity<List<Client>> getByClientName(@PathVariable String nameClient){
